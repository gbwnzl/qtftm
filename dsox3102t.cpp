...
bool DSOX3102T::testConnection()
{
    ...
    if (!resp.startsWith("AGILENT TECHNOLOGIES,DSO-X 2014A")) {
    ...
    resp = p_comm->queryCmd(":SYSTem:ERRor?\n");
    if (!resp.isEmpty()) {
        emit logMessage(QString("Error code: %1").arg(QString(resp)));
    }
    ...
    emit logMessage(QString("HELLOOOO"));
...
void DSOX3102T::configure()
{
    ...
    // Channel 1 config
    p_comm->writeCmd(":CHANNEL1:COUPLING AC");
    p_comm->writeCmd(":CHANNEL1:OFFSET 0.0");
    p_comm->writeCmd(":CHANNEL1:SCALE 0.5");
    p_comm->writeCmd(":CHANNEL1:UNITs VOLT");

    // Trigger config
    p_comm->writeCmd(":TRIGGER:MODE EDGE");
    p_comm->writeCmd(":TRIGGER:EDGE:SLOPE NEGATIVE");
    p_comm->writeCmd(":TRIGGER:EDGE:SOURCE EXTernal");
    p_comm->writeCmd(":TRIGGER:EDGE:LEVEL 2.5,EXTernal");
    p_comm->writeCmd(":TRIGGER:SWEEP NORMAL");

    // Timebase & data setup
    p_comm->writeCmd(":TIMEBASE:MODE MAIN");
    p_comm->writeCmd(":TIMEBASE:REFERENCE LEFT");
    p_comm->writeCmd(":TIMEBASE:POSITION 0");
    // p_comm->writeCmd(":TIMEBASE:POSITION 100E-6");
    p_comm->writeCmd(":TIMEBASE:RANGE 1E-3");

    p_comm->writeCmd(":WAVEFORM:POINTS 1000");
    ...
    p_comm->writeCmd(":ACQUIRE:TYPE HRESolution");
    p_comm->writeCmd(":RUN");
...
void DSOX3102T::setResolution()
{
    ...
    switch (r) {
    case QtFTM::Res_1kHz:
        p_comm->writeCmd(":TIMEBASE:RANGE 1000E-6;:ACQUIRE:POINTS 1000"); //2000000
        break;
    case QtFTM::Res_2kHz:
    ...
    case QtFTM::Res_10kHz:
        ...
        break;
    default:
        p_comm->writeCmd(":TIMEBASE:RANGE 200E-6;:ACQUIRE:POINTS 1000"); //2000000
        break;
    }
    ...
    p_comm->writeCmd(QString(":RUN"));
    // p_comm->writeCmd(QString(":TRIGGER:FORCE"));
    p_comm->writeCmd(QString(":DIGITIZE CHAN1"));
    ...
    // d_waveformPrefix = p_comm->queryCmd(QString(":WAVeform:PREamble?\n"));
